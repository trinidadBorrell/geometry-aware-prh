# exp-004: neighbourhood size k â€” does local similarity become global?

**Status:** done Â· **Owner:** oddharak Â· **Outputs:** `/workspace/results/oddharak/exp-004-knn-k-sweep/`

## Idea

GrÃ¶ger et al. Fig. 26 sweeps the neighbourhood size of mutual kNN over k âˆˆ {10, 20, 50, 100} and
...
too: chance overlap of two random k-sets is â‰ˆ k/n. Two competing readings:
...
2. *(the objection)* mutual kNN keeps only set membership and discards distances, so as k â†’ n it
   saturates at 1 and carries no geometry. The metric that genuinely interpolates local â†’ global
   is CKNNA, which weights by centred kernel values and provably recovers CKA at k = n
   (Huh et al., Appendix A, Eq. 18â€“19). Then mutual kNN's calibrated score should *collapse*
...
comparison uses normalised quantities â€” excess over the calibrated threshold Ï„ and the z-score
(raw âˆ’ Î¼â‚€)/Ïƒâ‚€ â€” alongside the paper-style gated score.

## Method

- Metrics: mutual kNN (k = 200) and CKNNA (k = 200); CKNNA is the localâ†’global bridge, with the
  k = 10 results of exp-003 and linear CKA as the two reference points.
- 200 permutations, Î± = 0.05, q = 0.95 outlier clamp, max over layer pairs â€” identical to
  exp-003, so the numbers are directly comparable.
- Reduced grid (40 pairs): 5 LLMs spanning 0.56B â†’ 13B across three families, 8 ViTs covering
...
- Mutual kNN calibration uses the mask-based variant, whose cost does not depend on k
  (the index-based one would allocate an nÃ—kÃ—k tensor). `tests/test_prh_alignment.py` checks the
  two agree at k = 10.
...
Run on 2026-09-20, commit `5c1b36f`, RTX PRO 4500, ~20 min. Outputs (payloads, 14 PDFs from
Aristotelian's plotting, per-pair JSONL):
...
**The prediction held.** Raw mutual kNN rises 0.123 -> 0.349 and the calibration loss rises
11% -> 49%. The null threshold tau = 0.207 is almost exactly k/n = 200/1024 = 0.195, i.e. chance
overlap of two random 200-sets: the extra raw score is mostly chance, as expected.

**But mutual kNN does not become CKA.** Its loss (49%) overshoots CKA's (29%) while its evidence
against the null collapses (z: 164 -> 63, the weakest of the four). The scaling trend also decays
(+0.87 -> +0.65), continuing the flattening Gröger et al. report up to k=100. Large-k mutual kNN
is not "global", it is noisy: it keeps set membership and discards distances, so the neighbourhood
stops carrying geometry. Hypothesis (2) is supported; (1) is not.

**CKNNA at k=200 is already essentially CKA.** tau 0.168 vs CKA's 0.157 and calibrated 0.261 vs
0.275: with n=1024, the kernel-weighted local metric has converged to the global one by k=200, as
Huh et al.'s k -> n argument predicts.
...
nulls and calibrated magnitudes. So restricting the kernel to neighbours preserves *which models
align better* even when the magnitude has gone global. That is a geometry effect, not a magnitude
effect, and it is the most promising thread for this project.

Caveats: 40 pairs only; the CLIP (INet ft) panel is empty because the reduced grid omits those
models; all p-values sit at the 0.005 floor of 200 permutations, so significance cannot be
